...
class Deezer:
    ...
    def get_track_download_url(self, track, quality=None, fallback=True, renew=False, **kwargs):
        ...
        try:
            ...
            # Try the newer media API (old e-cdns-proxy CDN hostnames have been decommissioned)
            track_token = track.get("TRACK_TOKEN")
            if track_token and self.license_token:
                media_result = self._get_media_url(track_token, quality, fallback)
                if media_result:
                    return media_result

            # Legacy fallback: generate CDN URL from MD5_ORIGIN (may not work if CDN is down)
    ...
    def _get_media_url(self, track_token, quality, fallback=True):
        """Gets the download URL via Deezer's media API (replacement for dead CDN endpoints)."""
        formats = [{"cipher": "BF_CBC_STRIPE", "format": quality}]
        if fallback:
            for q in track_formats.FALLBACK_QUALITIES:
                if q != quality:
                    formats.append({"cipher": "BF_CBC_STRIPE", "format": q})
        ...
        try:
            ...
            if "data" in result and result["data"]:
                ...
                if "media" in data and data["media"]:
                    media = data["media"][0]
                    if "sources" in media and media["sources"]:
                        url = media["sources"][0]["url"]
                        fmt = media.get("format", quality)
                        return (url, fmt)
        except Exception:
            pass
